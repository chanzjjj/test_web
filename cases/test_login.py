...
class TestLoginCase():
    ...
    def test_login_success(self, login_instance:UserLoginPage):
        ...
        # 获取实际结果
        acturl_result = login_instance.get_title()
        expect_result = "欢迎访问在线学习网——首页"
        assert acturl_result == expect_result
    ...
    def test_login_fail(self, login_instance:UserLoginPage):
        ...
        login_instance.click_login_btn()
        # 获取实际结果
        acturl_result = login_instance.get_tips()
        expect_result = "用户名或密码错误"
        assert acturl_result == expect_result
